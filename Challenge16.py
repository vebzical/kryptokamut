...
def EncryptMessage(key, msg):
	...
	quoted_msg = quote_from_bytes(msg).encode()
	...
	print([full_msg[i:i+16] for i in range(0, len(full_msg), 16)])
	iv = GenerateRandomIV()
	ciphertext = CBCEncrypt(16, full_msg, AES.new(key, AES.MODE_ECB), iv)
	
	return {"iv":iv, "ciphertext":ciphertext}

def decrypt_and_check_admin(key, ctxt):
...
xord = strxor(b'A'*16, b'AAAAA;admin=true')
crypted = EncryptMessage(key, b'A'*32)
crypted["ciphertext"] = strxor(b'\x00'*32+xord+b'\x00'*64, crypted["ciphertext"])

if decrypt_and_check_admin(key, crypted):
	print("wohoo")
